...
import { MdOutlineGroupAdd, MdOutlineSpaceDashboard } from "react-icons/md";
...
const Sidebar = () => {
  return (
    <div className='flex flex-col border-r-2 border-gray-800 min-h-full pt-6 text-gray-400'>
        ...
        <NavLink  to='/admin/addteacher' className={({isActive})=> `flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-64 cursor-pointer ${isActive && "bg-cyan-800 border-r-4 border-primary"}`}>
           <MdOutlineGroupAdd />
            <p className='hidden md:inline-block'>Add Teacher</p>
        </NavLink>
    </div>
  )
}
