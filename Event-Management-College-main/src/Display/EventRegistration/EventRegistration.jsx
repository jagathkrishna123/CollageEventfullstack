...
const EventRegistration = () => {
  ...
  useEffect(() => {
    ...
    const isAlreadyRegistered = allRegs.some(
      reg => reg.eventId === foundEvent.id && (
        reg.userId === loggedInUser.id ||
        (reg.teamData?.members?.some(m => m.regNo === loggedInUser.registerNumber))
      )
    );
                      ...
                      <div className="space-y-3">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            // Auto-lookup logic
                            if (index !== 0 && newRegNo.length > 0) {
                              const allUsers = JSON.parse(localStorage.getItem("registered_users") || "[]");
                              const foundUser = allUsers.find(u => u.registerNumber && u.registerNumber.toUpperCase() === newRegNo);

                              if (foundUser) {
                                handleMemberChange(index, "name", foundUser.name);
                              } else {
                                handleMemberChange(index, "name", ""); // Clear name if not found
                              }
                            } else if (index !== 0) {
